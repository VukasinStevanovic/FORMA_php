...
if ($akcija === 'obrisi' && $id > 0) {
    ...
    postaviti_flash('uspeh', 'Čas je obrisan.');
    header('Location: ' . BASE_URL . '/admin/casovi.php'); exit();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($vreme))      $greske[] = 'Unesite vreme časa.';
    ...
    if (empty($greske)) {
        ...
        if ($edit_id > 0) {
            ...
            postaviti_flash('uspeh', 'Čas je ažuriran.');
        } else {
            ...
            postaviti_flash('uspeh', 'Čas "' . $naziv . '" je dodan.');
        }
        header('Location: ' . BASE_URL . '/admin/casovi.php'); exit();
    }
...
if ($akcija === 'izmeni' && $id > 0) {
    ...
    $stmt->execute([$id]);
    $cas = $stmt->fetch();
    if (!$cas) { postaviti_flash('greska', 'Čas nije pronađen.'); header('Location: ' . BASE_URL . '/admin/casovi.php'); exit(); }
}
...
<head>
    ...
    <title>Časovi – Admin | Forma Fitness</title>
...
<div class="admin-layout">
    ...
    <main class="admin-main">
        <h1><?= in_array($akcija, ['novi','izmeni']) ? ($akcija === 'novi' ? 'Novi čas' : 'Izmeni čas') : '📅 Časovi' ?></h1>
        <?= prikazati_flash() ?>
        <?php foreach ($greske as $g): ?><div class="flash-poruka flash-greska"><?= e($g) ?></div><?php endforeach; ?>

        <?php if (in_array($akcija, ['novi', 'izmeni'])): ?>
            <a href="<?= BASE_URL ?>/admin/casovi.php" class="btn btn-outline btn-sm" style="margin-bottom:20px;display:inline-block;">← Nazad</a>
            <div class="p-box" style="max-width:500px;">
                <form method="POST">
                    ...
                    <div class="forma-group" style="display:flex;align-items:center;gap:10px;">
                        ...
                        <label for="aktivan" style="margin:0;">Aktivan čas</label>
                    </div>
                    <button type="submit" class="btn btn-primary"><?= $cas ? 'Sačuvaj' : 'Dodaj čas' ?></button>
                </form>
            </div>
        <?php else: ?>
            <div style="margin-bottom:16px;">
                <a href="?akcija=novi" class="btn btn-primary btn-sm">+ Novi čas</a>
            </div>
            <div class="tabela-wrap p-box" style="padding:0;">
                <table>
                    ...
                    <tbody>
                    <?php if (empty($casovi)): ?>
                        <tr><td colspan="7" style="text-align:center;color:var(--text2);padding:30px;">Nema časova</td></tr>
                    <?php else: ?>
                        ...
                        <tr>
                            ...
                            <td style="white-space:nowrap;">
                                ...
                                <a href="?akcija=obrisi&id=<?= $c['id'] ?>" class="btn btn-danger btn-sm"
                                   onclick="return confirm('Obrisati čas?')">Obriši</a>
                            </td>
                        </tr>
                        <?php endforeach; ?>
                    <?php endif; ?>
